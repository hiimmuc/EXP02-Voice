...
def pca_report_interactive(X, scale_X: bool = True, save_plot: bool = False):
    ...
    print(f"You can reduce from {dims_} to {dim_80} dimensions while retaining 80% of variance.")
    ...
    source_PCA = ColumnDataSource(
        data=dict(x_dim=dimensions_plot, y_cumvar=cumsum_plot, y_var=pca_evr_plot)
    )
    source_var95 = ColumnDataSource(data=dict(var95_x=[dim_95] * 96, var95_y=get_float_list(96)))
    source_twoD = ColumnDataSource(
        data=dict(twoD_x=[2] * (int(twoD) + 1), twoD_y=get_float_list(twoD + 1))
    )
    source_threeD = ColumnDataSource(
        data=dict(threeD_x=[3] * (int(threeD) + 1), threeD_y=get_float_list(threeD + 1))
    )
    ...
    p.line(
        "twoD_x", "twoD_y", line_width=0.5, line_dash="dotted", color="#435363", source=source_twoD
    )  # x=2
    p.line(
        "threeD_x",
        "threeD_y",
    ...
    p.vbar(
        x="x_dim", top="y_var", width=0.5, bottom=0, color="#D9F2EF", source=source_PCA, name="PCA"
    )
    ...
    # save and show p
    if save_plot:
        output_file("./outputs/PCA_analysis.html")
    show(p)

    # output PCA info as a dataframe
    df_PCA = pd.DataFrame(
        {"dimension": dimensions, "variance_cumulative": cumsum_, "variance": pca_evr}
    ).set_index(["dimension"])

    return df_PCA, X_pca, pca_evr


def pca_feature_correlation(
    X, X_pca, explained_var, features: list = None, fig_dpi: int = 150, save_plot: bool = False
):
    """
    ...
    if any(features):
        df_dotproduct.index = features

    # Normalize & Sort
    df_n, df_na, df_nabv = normalize_dataframe(
        df_dotproduct, explained_var, plot_opt=True, save_plot=save_plot
    )
...
def normalize_dataframe(
    df, explained_var=None, fig_dpi: int = 150, plot_opt: bool = True, save_plot: bool = False
):
    """
    ...
    if plot_opt:
        ...
        if save_plot:
            plt.savefig("./outputs/Z_normalized_corr_matrix.png")
        else:
            plt.show()
        ...
        if save_plot:
            plt.savefig("./outputs/Z_normalized_corr_matrix_Abs.png")
        else:
            plt.show()
    ...
    if explained_var.any():
        ...
        if plot_opt:
            ...
            if save_plot:
                plt.savefig("./outputs/Normalized_corr_matrix.png")
            else:
                plt.show()
    else:
        df_byvar = None
...
def pca_rank_features(df_nabv, verbose: bool = True):
    ...
    """
    df_rank = (
        df_nabv.copy().assign(score_=df_nabv.sum(axis=1)).sort_values("score_", ascending=False)
    )
...
def pca_full_report(X, features_: list = None, fig_dpi: int = 150, save_plot: bool = False):
    ...
    df_corr, df_n, df_na, df_nabv = pca_feature_correlation(
        X, X_pca, pca_evr, features_, fig_dpi, save_plot
    )
